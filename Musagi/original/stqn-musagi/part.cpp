...
*/
	...
	if(dui->DoButton(25, winh-70, "btn_pload", "load part content - will not affect the instrument"))
	{
		char filename[256];
		filename[0] = 0;
		if(dui->SelectFileLoad(filename, FILETYPE_PART))
		{
			FILE *file=fopen(filename, "rb");
			if(file)
			{
				if(!LoadContent(file))
					platform_MessageBox("Unknown part format (maybe incompatible version)", "Error", MESSAGEBOX_OK);
				fclose(file);
			}
			else
	...
	{
		char filename[256];
		if(dui->SelectFileSave(filename, FILETYPE_PART))
		{
			...
			if(file)
			{
				...
				platform_MessageBox("Unable to save (write protected?)", "Error", MESSAGEBOX_OK);
		}
	}
	...
	{
//		int choice=MessageBox("Are you sure you want to clear this part?", "Caution", MESSAGEBOX_OKCANCEL);
//		if(choice==IDOK)
			Clear();
