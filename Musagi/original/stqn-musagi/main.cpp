...
//					MessageBox((DWORD)hWndMain, "Unable to export song (write protected?)", "Error", MB_ICONEXCLAMATION);
...
#endif
	{
		...
		dui->DoKnob(176, 40, gConfig.speaker_volume, -1, "knob_spk", "speaker volume (adjust for pleasant listening)");
		audiostream->speaker_volume=pow(gConfig.speaker_volume, 2.0f);
	}
	...
	if(dui->DoButton(15, 32, "btn_srec", "record song to .wav file (automatic play/stop, loop off by default)"))
	{
		...
		{
			if(dui->SelectFileSave(filename, FILETYPE_WAVE))
			{
				diskrec=true;
	...
	if(dui->DoButton(kr_load_save_xpos+318-23, 31, "btn_save", "save everything") || autosave==40)
	{
		...
		if(autosave==40 && curfilename[0]!='\0')
		{
			...
			ok=dui->SelectFileSave(filename, FILETYPE_SONG);
		if(ok)
		{
			strcpy(curfilename, filename);
			if(!SaveSong(filename))
//				MessageBox((DWORD)hWndMain, "Unable to save song (write protected?)", "Error", MB_ICONEXCLAMATION);
				platform_MessageBox("Unable to save song (write protected?)", "Error", MESSAGEBOX_OK);
		}
	}
	...
	if(dui->DoButton(kr_load_save_xpos+318-23, 44, "btn_load", "load song - all changes will be lost!") || (dui->ctrl_down && (input->KeyPressed(SDLK_l) || input->KeyPressed(SDLK_o))))
	{
		LogPrint("main: loadbutton");
		thesong->Stop();
		char filename[256];
		filename[0] = 0;
		if(dui->SelectFileLoad(filename, FILETYPE_SONG))
		{
			curfilename[0]='\0';
			if(!LoadSong(filename))
//				MessageBox((DWORD)hWndMain, "Unknown song format (maybe incompatible version)", "Error", MB_ICONEXCLAMATION);
				platform_MessageBox("Unknown song format (maybe incompatible version)", "Error", MESSAGEBOX_OK);
			else
				strcpy(curfilename, filename);
		}
	}
	...
	dui->DrawLED(glkitGetWidth()-200+5, glkitGetHeight()-54, 0, gConfig.metronome_vol>0.f);
	dui->DoKnob(glkitGetWidth()-200+15, glkitGetHeight()-58, gConfig.metronome_vol, -1, "knob_metvol", "metronome volume (0=off)");

	dui->DrawText(glkitGetWidth()-200+38, glkitGetHeight()-38, dui->palette[6], "tooltips");
	dui->DrawLED(glkitGetWidth()-200+5, glkitGetHeight()-39, 0, gConfig.tooltips_on==1);
	if(dui->DoButton(glkitGetWidth()-200+15, glkitGetHeight()-39, "btn_toolt", "toggle tooltips (like this one)"))
		gConfig.tooltips_on=!gConfig.tooltips_on;
	...
	dui->DrawLED(glkitGetWidth()-90+5, glkitGetHeight()-39, 0, gConfig.scrollfollow==1);
	if(dui->DoButton(glkitGetWidth()-90+15, glkitGetHeight()-39, "btn_scrfo", "scroll to follow play position"))
		gConfig.scrollfollow=!gConfig.scrollfollow;
	...
	dui->DrawText(glkitGetWidth()-200+5, glkitGetHeight()-20, dui->palette[6], "%i", gConfig.kbkboctave);
...
//		int choice=MessageBox(hWndMain, "Are you sure you want to erase everything and start from scratch?", "Caution", MB_ICONEXCLAMATION|MB_OKCANCEL);
		if(platform_MessageBox("Are you sure you want to erase everything and start from scratch?", "Caution", MESSAGEBOX_OKCANCEL))
		{
	...
	if(dui->CheckRCResult(51) || dui->CheckRCResult(32))
	{
		char filename[256];
		filename[0] = 0;
		if(dui->SelectFileLoad(filename, FILETYPE_INST))
		{
			FILE *file=fopen(filename, "rb");
			if(file==NULL)
				platform_MessageBox("File not found", "Error", MESSAGEBOX_OK);
			else
			{
				...
				if(!LoadInstrument(&gearstacks[fcurstack], file, false))
					platform_MessageBox("Unknown instrument format (maybe incompatible version)", "Error", MESSAGEBOX_OK);
				else
				{
	...
	{
		char filename[256];
		if(dui->SelectFileSave(filename, FILETYPE_INST))
		{
			...
			if(file!=NULL)
			{
				...
				platform_MessageBox("Unable to save instrument (write protected?)", "Error", MESSAGEBOX_OK);
			if(file!=NULL)
				fclose(file);
		}
	}
	if(dui->CheckRCResult(34))
	{
		...
		else
		{
			...
			platform_MessageBox(string, "Info", MESSAGEBOX_OK);
		}
	}
	...
	if(dui->CheckRCResult(21) || (current_part==NULL && dui->ctrl_down && input->KeyPressed(SDLK_c))) // copy spart
	{
		copyclone_choice=1;
		dui->CloseMenu();
	}
	if(dui->CheckRCResult(22) || (current_part==NULL && dui->ctrl_down && input->KeyPressed(SDLK_d))) // clone spart
	{
		copyclone_choice=2;
		dui->CloseMenu();
	}
	...
	if(menu_create_part || (((dui->lmbclick && thesong->ghostpart_mode>0) || dui->mmbclick) && mouse_in_song_area)) // add part
	{
		...
		{
			if(gearstacks[fcurstack].instrument==NULL)
				platform_MessageBox("Select an instrument first.", "Info", MESSAGEBOX_OK);
			else
			{
				mungoparts[i]=new Part(thesong->GenGuid());
	...
	if(input->KeyPressed(SDLK_SPACE)) // space to play part, conflicts with space to scroll/pan
	{
		if(!shortkeydown)
		{
			if(current_part!=NULL)
	...
	if(rcresult!=-1)
	{
...
#ifdef MIDI
		if(rctype>=9 && rctype<=11 && gConfig.midiInputDevice!=-1) // change midi input device (unless disabled)
		{
			gConfig.midiInputDevice=rcresult-rctype*10-1;
			midi_init(gConfig.midiInputDevice);
		}
#endif
		...
		if(rctype==1) // add instrument
		{
			...
			if(gearstacks[fcurstack].instrument!=NULL)
			{
				if(platform_MessageBox("Adding a new instrument here will replace the current one, are you sure?", "Caution", MESSAGEBOX_OKCANCEL))
					ok=false;
				else
				{
					audiostream->RemoveGearStack(&gearstacks[fcurstack]);
			...
			if(ok)
			{
				bool has_gapan=true;
#ifndef MIDI
				// Skip non-existent menu entries
				if(rcresult-10>=5)
					rcresult+=2;
#endif
#ifndef VST
				// Skip non-existent menu entry
				if(rcresult-10>=7)
					rcresult++;
#endif
				switch(rcresult-10)
...
bool glkCalcFrame()
{
	...
	bool kbkbhit=false;
	for(int i=0;i<(int)NUM_KB_KEYS;i++)
	{
#ifdef _WIN32
		if(input->KeyPressed(kbkbk[i]) || (InChildWindow()!=NULL && (GetAsyncKeyState(kbkbc[i])&KF_UP)))
#else
		// Not sure what the InChildWindow() thing is supposed to be for
		if(input->KeyPressedScancode(g_KeyboardFrequencies[i].scancode) /*|| (InChildWindow()!=NULL && (GetAsyncKeyState(kbkbc[i])&KF_UP))*/)
#endif
		{
			kbkbsomething=true;
			kbkbhit=true;
			...
			{
				kbkbkeydown[i]=true;
				Keyboard_KeyDown(g_KeyboardFrequencies[i].frequency+gConfig.kbkboctave*12);
			}
		}
		else
		{
			if(kbkbkeydown[i])
			{
				Keyboard_KeyUp(g_KeyboardFrequencies[i].frequency+gConfig.kbkboctave*12);
			}
			kbkbkeydown[i]=false;
		}
	}
...
void glkPreInit()
{
	...
	//glkit_winx=gConfig.winx;	// unused with SDL...
	//glkit_winy=gConfig.winy;
	glkit_width=gConfig.winwidth;
	glkit_height=gConfig.winheight;
	//glkit_winmax=gConfig.winmax;
...
/**
 * Allocate and initialise a new song. Does not free the current song.
 */
void Reset()
{
...
#ifdef MIDI
	LogPrint("init midi");
	if(gConfig.midiInputDevice==-1)
		LogPrint("midi disabled in config");
	else if(!midi_init(gConfig.midiInputDevice))
		LogPrint("ERROR: midi failed");
#endif

	LogPrint("portaudio environment variable");
	platform_set_portaudio_latency(gConfig.portaudio_latency);
...
void glkFree()
{
	...
	//gConfig.winx=glkit_winx;
	//gConfig.winy=glkit_winy;
	gConfig.winwidth=glkit_width;
	gConfig.winheight=glkit_height;
	//gConfig.winmax=glkit_winmax;
