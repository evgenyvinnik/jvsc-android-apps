...
public:
	...
	DUI()
	{
		...
		FileSelectorInit(); // probably shouldn't be here
	...
	char* mstrcat(char* temp, const char* str1, const char* str2)
	{
		strcpy(temp, str1);
		strcat(temp, str2);
		return temp;
...
//			LogPrint("ONE DOUBLE CLICK");
		...
		delete_pressed=input->KeyPressed(SDLK_DELETE);
		space_down=input->KeyPressed(SDLK_SPACE);
		pageup=input->KeyPressed(SDLK_PAGEUP);
		pagedown=input->KeyPressed(SDLK_PAGEDOWN);
		f1_down=input->KeyPressed(SDLK_F1);
		f2_down=input->KeyPressed(SDLK_F2);
		f3_down=input->KeyPressed(SDLK_F3);
		f4_down=input->KeyPressed(SDLK_F4);
		f5_down=input->KeyPressed(SDLK_F5);
		if(editstring)
		{
			key_a=false;
			key_c=false;
...
//		glkmouse->glk_mousemiddleclick=false;
	...
	float* DoOverlay(int& choice) // returns right-click menu choice, if any.... uh, that's ugly :P
	{
		...
		else if(gConfig.tooltips_on || ctrl_down)
		{
	...
	void EnterWindow(const char *winid, int x, int y, int sx, int sy, int w, int h)
	{
		window_level++;
	...
	void NameSpace(void *identifier)
	{
		if(identifier==NULL)
			...
			sprintf(namestring, "%.8X", (Uint32)((uintptr_t)identifier&0xffffffff));
	};

	bool DoButtonLogic(int x, int y, int w, int h, float& value, const char *idstr) // Use config file thing instead of manual coordinates/sizes?
	{
	...
	bool DoButton(int x, int y, const char *idstr, const char *helptext)
	{
		if(MouseInZone(x+gui_winx, y+gui_winy, 20, 10))
			strcpy(popup_text, helptext);
		return DoButton(x, y, false, idstr);
	};
	bool DoButtonRed(int x, int y, const char *idstr, const char *helptext)
	{
		if(MouseInZone(x+gui_winx, y+gui_winy, 20, 10))
			strcpy(popup_text, helptext);
		return DoButton(x, y, true, idstr);
	};
	bool DoButton(int x, int y, bool red, const char *idstr)
	{
	...
	bool DoKnob(int x, int y, float& value, int jax, const char *idstr, const char *helptext)
	{
		if(MouseInZone(x+gui_winx, y+gui_winy, 16, 16))
			strcpy(popup_text, helptext);
		return DoKnob(x, y, value, jax, idstr);
	};
	bool DoKnob(int x, int y, float& value, int jax, const char *idstr)
	{
		strcpy(curname, idstr);
		SpaceName();
		idstr=curname;
	...
	void DoPartIcon(int x, int y, int w, int& start, int& ypos, float zoom, Color& color, Color& instcol, const char *name, int mode, bool clone, const char *idstr)
	{
		int framecol=6;
		bool active=false;
		if(mode==0)
	...
	void RequestWindowFocus(const char *idstr)
	{
		strcpy(curname, idstr);
		SpaceName();
		idstr=curname;
	...
	void DropWindowFocus(const char *idstr)
	{
		strcpy(curname, idstr);
		SpaceName();
	...
	bool StartWindowSnapX(int& x, int& y, int& snapx, int w, int h, const char *title, const char *idstr)
	{
		int px=x;
		snapx=x;
		bool snapped=false;
	...
	bool StartWindow(int& x, int& y, int w, int h, const char *title, const char *idstr)
	{
		return StartWindow(x, y, false, w, h, false, w, h, h, title, idstr);
	};
	bool StartWindowResize(int& x, int& y, int& w, int& h, int minw, int minh, int maxh, const char *title, const char *idstr)
	{
		return StartWindow(x, y, false, w, h, true, minw, minh, maxh, title, idstr);
	};
	bool StartWindow(int& x, int& y, bool xsnap, int& w, int& h, bool resize, int minw, int minh, int maxh, const char *title, const char *idstr)
	{
	...
	void StartFlatWindow(int x, int y, int w, int h, const char *idstr)
	{
		strcpy(curname, idstr);
		SpaceName();
		idstr=curname;
	...
	void StartScrollArea(int x, int y, int w, int h, float& scrollx, float& scrolly, int fw, int fh, Color& color, const char *idstr)
	{
		strcpy(curname, idstr);
		SpaceName();
		idstr=curname;
	...
	void ShadowScrollArea(int x, int y, int w, int h, float& scrollx, float& scrolly, int fw, int fh, Color& color, const char *idstr)
	{
		strcpy(curname, idstr);
		SpaceName();
		idstr=curname;
	...
	void DrawText(int sx, int sy, Color& color, const char *string, ...)
	{
		glEnable(GL_BLEND);
...
/*			int yo=0;
	...
	int VStringLength(const char *string, ...)
	{
		char string2[256];
		va_list args;
...
/*			if(ch=='Å') ch='~'+2;
	...
	void DrawBar(int x, int y, int w, int h, const Color& color)
	{
		glDisable(GL_BLEND);
		x+=gui_winx;
		y+=gui_winy;
	...
	bool SelectFileSave(char *filename, FileType type)
	{
		glkitShowMouse(true);
		bool chosen=false;
		chosen=FileSelectorSave(filename, type);
		//glkitShowMouse(false);
		return chosen;
	};

	bool SelectFileLoad(char *filename, FileType type)
	{
		glkitShowMouse(true);
		bool chosen=false;
		chosen=FileSelectorLoad(filename, type);
		//glkitShowMouse(false);
		return chosen;
	};

	bool SelectFileLoad(char *filename, FileType type, char *title)
	{
		glkitShowMouse(true);
		bool chosen=false;
		chosen=FileSelectorLoad(filename, type, title);
		//glkitShowMouse(false);
		return chosen;
	};
};
