...
bool LoadInstrument(GearStack *gs, FILE *file, bool songload)
{
	...
	if(instrument==NULL) // unknown instrument
	{
		...
		if(songload && errorcount<5)
		{
			...
			platform_MessageBox(string, "Warning", MESSAGEBOX_OK);
			errorcount++;
		}
		else
...
//		LogPrint("loading part %i", index);
	...
	if(errorcount>=5)
		platform_MessageBox("Too many errors. If you want more details - quit the program, enable logging in config.txt, reload the song, then read log.txt", "Warning", MESSAGEBOX_OK);
	errorcount=0;
